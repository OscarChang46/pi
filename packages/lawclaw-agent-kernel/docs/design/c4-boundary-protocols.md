...
|---|---|---|---|---|---|
| `BND-EXT-001` | Client/Backend ↔ L1 | 本机进程边界 | `AgentKernelProtocolV1` → `AgentSystemGateway` | CLI 进程内调用；Desktop Host 使用 stdio JSONL；Loopback HTTP/SSE 可选 | 协议外观稳定；Transport 可替换；命令幂等；事件可补拉 |
