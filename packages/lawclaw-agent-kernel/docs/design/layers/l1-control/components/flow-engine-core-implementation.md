...
- [Core入口](../../../../../src/control/flow-engine/flow-engine.ts)
...
2026-09-07：继续检查迁移动作及Factory中的类型分派，将模型输出、权限结果和计划绑定校验改为穷尽策略表，分离工具/模型恢复处理。检查结果和保留条件的依据见[分支审查](flow-engine-branch-audit.md)。新增AK-FE-017/018后，18个选中用例及包内`--checks`通过，根目录`npm run check`通过。前文14/16项为历史验证记录；运行边界接入范围未改变。
