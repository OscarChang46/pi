...
import type { RequestContext, TimePoint, TimePort, ToolDescriptor, ToolPolicy } from "../src/contracts/index.ts";
import {
	type AuthorizationSnapshot,
	type AuthorizationSnapshotPort,
	computeArgumentsDigest,
	computeToolDescriptorDigest,
	type FrozenAgentRunScope,
...
class FakeTimePort implements TimePort {
	...
	public constructor(isoUtc: string) {
		this.#epochMilliseconds = Date.parse(isoUtc);
	}
	...
	public now(): TimePoint {
		return { epochMilliseconds: this.#epochMilliseconds, isoUtc: new Date(this.#epochMilliseconds).toISOString() };
	}
	...
	public parseIsoUtc(value: string): TimePoint | undefined {
		...
		if (!Number.isFinite(epochMilliseconds) || new Date(epochMilliseconds).toISOString() !== value) return undefined;
		return { epochMilliseconds, isoUtc: value };
	}

	public addMilliseconds(base: TimePoint, deltaMilliseconds: number): TimePoint {
		const epochMilliseconds = base.epochMilliseconds + deltaMilliseconds;
	...
	public toZonedDateTime(): never {
		throw new Error("测试不使用本地时间投影");
	}
}
...
test("只对五层权限交集内的调用返回不可变 ALLOW grant", async () => {
...
test("任一上游权限收窄都不能增加最终 Grant", async () => {
	const { time, service } = harness();
...
test("工具、网络、Secret 和预算分别按交集默认拒绝", async () => {
...
test("端口异常、快照不匹配、人工确认和取消均失败关闭", async () => {
...
test("revalidate 校验 digest、有效期和 policy/auth epoch", async () => {
	const { time, service } = harness();
	const decision = await service.evaluate(context(time), scope(time), request(), new AbortController().signal);
	assert.equal(decision.kind, "ALLOW");
	if (decision.kind !== "ALLOW") return;
...
test("authorization epoch 变化使尚未过期的 Grant 失效", async () => {
	const time = new FakeTimePort("2026-09-03T00:00:00.000Z");
