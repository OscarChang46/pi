...
import type { AgentRunExecutor } from "../../src/control/agent-run.ts";
...
import { RunRegistry } from "../../src/control/run-registry.ts";
...
test("[AK-RUN-001] 控制入口独立持有 Run，Session 只关联 Run 标识", async () => {
	...
	assert.equal(session?.runIds.length, 2);
...
test("[AK-RUN-002] Runtime 和 Session 拒绝跨租户复用、重复 Run 与容量扩张", async () => {
	const runtime = new AgentSystem(
		"runtime:test",
		"agent:test",
		new TwoLoopExecutor(),
