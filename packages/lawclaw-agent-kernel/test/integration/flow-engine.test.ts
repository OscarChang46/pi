...
import type { FlowRunAction, FlowRunState } from "../../src/contracts/control/flow-engine/flow-engine-contract.ts";
import { resolveFlowRunTransition } from "../../src/contracts/control/flow-engine/flow-engine-transitions.ts";
import { FLOW_RUN_ACTION } from "../../src/contracts/control/flow-engine/flow-engine-values.ts";
import type { FlowGraphDefinition } from "../../src/contracts/control/flow-engine/flow-graph.ts";
import { ActivityInterceptor } from "../../src/control/flow-engine/activity-interceptor.ts";
import { FlowEngine } from "../../src/control/flow-engine/flow-engine.ts";
import { TaskGraph } from "../../src/control/flow-engine/task-graph.ts";
...
test("[AK-FS-013] FlowRun命名变更保持v1身份与Completed回放，拒绝混入AgentRun字段", async (t) => {
	...
	const flowRun = journal.transition(journal.admit(request), FLOW_RUN_ACTION.START, "STARTED");
	...
	journal.transition(journal.get(flowRun.flowRunId), FLOW_RUN_ACTION.YIELD, "WAITING");
...
test("[AK-FS-001] 四态完整矩阵拒绝非法迁移和终态出边", () => {
	const states: FlowRunState[] = ["Ready", "Running", "Yield", "Terminate"];
	const actions: FlowRunAction[] = ["start", "yield", "resume", "recover", "terminate"];
	...
	for (const state of states)
		for (const action of actions) {
			...
			if (target) assert.equal(resolveFlowRunTransition(state, action), target);
			else assert.throws(() => resolveFlowRunTransition(state, action), /INVALID_TRANSITION/);
		}
});
