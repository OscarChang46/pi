...
import { FLOW_RUN_ACTION, FLOW_RUN_STATE } from "../../contracts/control/flow-engine/flow-engine-values.ts";
...
export class FlowEngine {
	...
	async run(request: FlowRunRequest, workflow: FlowWorkflow): Promise<FlowRun> {
		...
		if (flowRun.state === FLOW_RUN_STATE.TERMINATE || flowRun.state === FLOW_RUN_STATE.RUNNING) return flowRun;
		if (flowRun.state === FLOW_RUN_STATE.YIELD)
			flowRun = this.#journal.transition(flowRun, FLOW_RUN_ACTION.RESUME, "RESUMED");
		flowRun = this.#journal.transition(flowRun, FLOW_RUN_ACTION.START, "STARTED");
		...
		try {
			...
			return this.#journal.transition(
				this.#journal.get(flowRun.flowRunId),
				error instanceof FlowYield || unfinished ? FLOW_RUN_ACTION.YIELD : FLOW_RUN_ACTION.TERMINATE,
				reason,
			);
		}
	}
	...
	recover(flowRunId: string): FlowRun {
		return this.#journal.transition(this.#journal.get(flowRunId), FLOW_RUN_ACTION.RECOVER, "EXECUTION_INTERRUPTED");
	}
	/** 显式终止；外部已发出的调用不能撤回，后续回调被代次校验拒绝。 */
	terminate(flowRunId: string, reason: string): FlowRun {
		const flowRun = this.#journal.get(flowRunId);
		return flowRun.state === FLOW_RUN_STATE.TERMINATE
			? flowRun
			: this.#journal.transition(flowRun, FLOW_RUN_ACTION.TERMINATE, reason);
	}
