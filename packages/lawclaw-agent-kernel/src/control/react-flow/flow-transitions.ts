...
import { canonicalize, flowId } from "../../contracts/flow-value.ts";
import {
	FLOW_EFFECT,
	FLOW_WAIT_REASON,
	REACT_FLOW_STATE,
...
type ModelInput = TransitionInput<typeof REACT_FLOW_STATE.AWAITING_MODEL, "ModelCompleted">;
...
const resolveModelOutput = createVariantMatcher<ModelOutput, ModelContext, Resolution>({
	answer: (output, { input, append }) => {
		...
		return output.outputRef.bytes > budget.maxOutputBytes
			...
			: plan({ kind: REACT_FLOW_STATE.COMPLETED, outputRef: output.outputRef }, [], append, []);
	},
	tools: (output, { input, append }) => {
		...
		return plan(
			{ kind: REACT_FLOW_STATE.AWAITING_PERMISSION, proposalId: proposal.proposalId, commandOrdinal: 0 },
	...
	child: (output, { input, append }) => {
		...
		return plan(
			{ kind: REACT_FLOW_STATE.AWAITING_CHILD, childId, commandOrdinal: 0 },
...
type PermissionContext = {
	input: TransitionInput<typeof REACT_FLOW_STATE.AWAITING_PERMISSION, "PermissionResolved">;
...
const resolvePermissionResult = createVariantMatcher<PermissionResult, PermissionContext, Resolution>({
	ask: (result, { input, first }) =>
		transitionContext(input).plan({
			kind: REACT_FLOW_STATE.SUSPENDED,
			reason: { kind: FLOW_WAIT_REASON.APPROVAL, approvalRef: result.approvalRef, proposalId: first.proposalId },
		}),
	...
	allow: (result, { input, first }) => {
		...
		return plan(
			{ kind: REACT_FLOW_STATE.AWAITING_TOOL, proposalId: first.proposalId, commandOrdinal: 0 },
...
type ReconciliationInput = TransitionInput<
	typeof REACT_FLOW_WAIT_STATE.TOOL_UNKNOWN | typeof REACT_FLOW_WAIT_STATE.MODEL_UNKNOWN,
	"EffectReconciled"
>;
...
export const FLOW_TRANSITIONS = Object.freeze([
	defineTransition(
		REACT_FLOW_STATE.READY,
		"AdvanceRequested",
		[REACT_FLOW_STATE.AWAITING_PERMISSION, REACT_FLOW_STATE.AWAITING_MODEL, REACT_FLOW_STATE.FAILED],
		(input) => {
			...
			if (first)
				return plan(
					{ kind: REACT_FLOW_STATE.AWAITING_PERMISSION, proposalId: first.proposalId, commandOrdinal: 0 },
					[{ kind: "RequestPermission", proposal: first }],
				);
			if (contextFailure !== null)
				return failed(
					contextFailure === "unavailable" ? "FLOW_CONTEXT_UNAVAILABLE" : "FLOW_CONTEXT_LIMIT_EXCEEDED",
				);
			if (
				context === null ||
			...
			return plan(
				{ kind: REACT_FLOW_STATE.AWAITING_MODEL, commandOrdinal: 0 },
	...
	defineTransition(
		REACT_FLOW_STATE.AWAITING_MODEL,
		"ModelCompleted",
		[
			REACT_FLOW_STATE.COMPLETED,
			REACT_FLOW_STATE.AWAITING_PERMISSION,
			REACT_FLOW_STATE.AWAITING_CHILD,
	...
	defineTransition(
		REACT_FLOW_STATE.AWAITING_MODEL,
		"ModelFailed",
		[REACT_FLOW_WAIT_STATE.MODEL_UNKNOWN, REACT_FLOW_STATE.FAILED],
		(input) => {
			const { run, event } = input;
			const { position } = run;
			const payload = event.payload;
			...
			if (payload.modelCommandId !== position.commandId || event.causationId !== position.commandId)
				return conflict();
			if (payload.effect === FLOW_EFFECT.UNKNOWN || payload.effect === FLOW_EFFECT.KNOWN_APPLIED) {
				...
				return plan(
					{
						kind: REACT_FLOW_STATE.SUSPENDED,
						reason: { kind: FLOW_WAIT_REASON.MODEL_UNKNOWN, modelCommandId: position.commandId, incidentRef },
					},
	...
	defineTransition(
		REACT_FLOW_STATE.AWAITING_PERMISSION,
		"PermissionResolved",
		[REACT_FLOW_WAIT_STATE.APPROVAL, REACT_FLOW_STATE.AWAITING_TOOL, REACT_FLOW_STATE.FAILED],
		(input) => {
			const { run, event } = input;
	...
	defineTransition(
		REACT_FLOW_STATE.AWAITING_TOOL,
		"ToolObserved",
		[REACT_FLOW_WAIT_STATE.TOOL_UNKNOWN, REACT_FLOW_STATE.READY, REACT_FLOW_STATE.FAILED],
		(input) => {
			const { run, event } = input;
			...
			if (
				...
				return conflict();
			if (payload.effect === FLOW_EFFECT.UNKNOWN) {
				const incidentRef = flowId("inc", {
					runId: run.runId,
					targetCommandId: position.commandId,
					kind: "unknown",
				...
				return plan(
					{
						kind: REACT_FLOW_STATE.SUSPENDED,
						reason: { kind: FLOW_WAIT_REASON.TOOL_UNKNOWN, toolCommandId: position.commandId, incidentRef },
					},
					[{ kind: "RequestReconciliation", targetCommandId: position.commandId, incidentRef }],
				);
			}
	...
	defineTransition(
		REACT_FLOW_STATE.AWAITING_CHILD,
		"ChildCompleted",
		[REACT_FLOW_STATE.READY, REACT_FLOW_STATE.FAILED],
		(input) => {
			const { run, event } = input;
			const { position, budget } = run;
			const payload = event.payload;
	...
	defineTransition(
		REACT_FLOW_WAIT_STATE.APPROVAL,
		"ApprovalResolved",
		[REACT_FLOW_STATE.AWAITING_PERMISSION, REACT_FLOW_STATE.FAILED],
		(input) => {
			const { run, event } = input;
			const { position, pendingActions } = run;
			const payload = event.payload;
			...
			return payload.approved
				? plan({ kind: REACT_FLOW_STATE.AWAITING_PERMISSION, proposalId: first.proposalId, commandOrdinal: 0 }, [
						{ kind: "RequestPermission", proposal: first },
					])
	...
	defineTransition(
		REACT_FLOW_WAIT_STATE.TOOL_UNKNOWN,
		"EffectReconciled",
		[REACT_FLOW_STATE.READY, REACT_FLOW_STATE.FAILED],
		(input) => {
	...
	defineTransition(
		REACT_FLOW_WAIT_STATE.MODEL_UNKNOWN,
		"EffectReconciled",
		[REACT_FLOW_STATE.COMPLETED, REACT_FLOW_STATE.FAILED],
		(input) => {
			...
			return plan({ kind: REACT_FLOW_STATE.COMPLETED, outputRef: result }, [], [entry("assistant", result)], []);
		},
	),
]);
