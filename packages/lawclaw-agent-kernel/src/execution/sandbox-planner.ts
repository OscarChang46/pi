...
import type { EgressClaim, PermissionGrant } from "../contracts/permissions.ts";

/** 工具执行守卫要求 SandboxPlanner 落实的权限子集。 */
export interface SandboxPlanRequirement {
	/** Composition Root 选择的沙箱能力档案及版本。 */
...
function _egressKey(grant: SandboxEgressGrant): string {
...
export class SandboxPlanner {
	...
	/** 将现有 Grant 收敛为沙箱执行计划；资源或预算扩张抛出 KernelError，不创建沙箱。 */
