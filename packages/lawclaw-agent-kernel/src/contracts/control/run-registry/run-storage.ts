...
/** 同步本地受理明确拒绝：事务未写入，调用已结束，不存在迟到提交。 */
export class RunAdmissionRejected extends Error {
	/** 保留明确的领域拒绝码；I/O 或提交确认未知不得使用此类型。 */
	constructor(code: string) {
		super(code);
		this.name = "RunAdmissionRejected";
	}
}

/** 持久化适配器复用Core验证和摘要规则；由装配方注入，不依赖控制层实现。 */
export interface RunStorageRules {
...
export interface AgentRunStore extends StateCommitPort {
	/** 耐久受理完整初始输入；同ID异内容拒绝。 */
	...
	/** 读取产生指定模型命令的原子提交中固定的候选引用；不读取最新组装结果。 */
