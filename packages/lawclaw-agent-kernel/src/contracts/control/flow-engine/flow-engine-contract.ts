import type { FLOW_JOURNAL_EVENT, FLOW_RUN_ACTION, FLOW_RUN_STATE } from "./flow-engine-values.ts";
/** 系统原子状态；业务位置和终止原因不扩展此集合。 */
export type FlowRunState = (typeof FLOW_RUN_STATE)[keyof typeof FLOW_RUN_STATE];

/** 生命周期事件；恢复必须显式撤销旧运行代次。 */
export type FlowRunAction = (typeof FLOW_RUN_ACTION)[keyof typeof FLOW_RUN_ACTION];

/** 一次逻辑运行的不可变身份。 */
export interface FlowRunRequest {
	/** 租户内唯一的运行标识。 */
	readonly flowRunId: string;
	/** 调用方固定的工作流版本，恢复时必须一致；框架不内置注册中心。 */
...
export interface FlowJournal {
	...
	/** CAS追加状态事件；未声明边拒绝。 */
	transition(flowRun: FlowRun, action: FlowRunAction, reason: string, result?: unknown): FlowRun;
	/** 校验Running及当前执行代次。 */
