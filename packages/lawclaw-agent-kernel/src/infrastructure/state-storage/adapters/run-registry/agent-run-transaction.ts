import type { RunStorageRules } from "../../../../contracts/control/run-registry/run-storage.ts";
import type { AdvanceInput, ExecutionClaim } from "../../../../contracts/flow-engine.ts";
import type { TimePort } from "../../../../contracts/index.ts";
import { AgentRunDatabase } from "./agent-run-database.ts";

/** 共享同一SQLite连接、作用域及快照规则，跨表提交不得换连接。 */
export class AgentRunTransaction {
	/** 由存储门面拥有的连接。 */
	readonly db: AgentRunDatabase;
	/** 固定可信作用域。 */
	readonly scope: string;
	/** 注入的Core校验与摘要协议。 */
	readonly rules: RunStorageRules;
	/** 可信时钟。 */
	readonly time: TimePort;
	/** 建立连接；空作用域拒绝。 */
	constructor(path: string, scope: string, rules: RunStorageRules, time: TimePort) {
		if (!scope) throw new Error("FLOW_STORAGE_SCOPE_REQUIRED");
		this.db = new AgentRunDatabase(path);
		this.scope = scope;
		this.rules = rules;
		this.time = time;
	}
