...
import { resolveFlowRunTransition } from "../../../../contracts/control/flow-engine/flow-engine-transitions.ts";
import { FLOW_JOURNAL_EVENT, FLOW_RUN_STATE } from "../../../../contracts/control/flow-engine/flow-engine-values.ts";
...
export class SqliteFlowJournal implements FlowJournal {
	...
	get(flowRunId: string): FlowRun {
		...
		for (const event of events) {
			if (event.kind === FLOW_JOURNAL_EVENT.STATE_CHANGED) {
				const data = event.payload as { action: FlowRunAction; reason: string; result: unknown };
				const state = resolveFlowRunTransition(flowRun.state, data.action);
				flowRun = {
					...flowRun,
					state,
					epoch: state === FLOW_RUN_STATE.RUNNING ? event.sequence : flowRun.epoch,
					reason: data.reason,
					result: data.result,
				};
			}
	...
	transition(flowRun: FlowRun, action: FlowRunAction, reason: string, result: unknown = null): FlowRun {
		validateKey(reason);
		return this.#db.transaction(() => {
			const actual = this.get(flowRun.flowRunId);
			if (actual.revision !== flowRun.revision) throw new Error("FLOW_REVISION_CONFLICT");
			resolveFlowRunTransition(actual.state, action);
	...
	assertActive(token: FlowExecutionToken): void {
		...
		if (flowRun.state !== FLOW_RUN_STATE.RUNNING || flowRun.epoch !== token.epoch)
	...
	reconcile(flowRunId: string, activity: FlowActivity, result: unknown, evidence: string): void {
		...
		this.#db.transaction(() => {
			if (this.get(flowRunId).state !== FLOW_RUN_STATE.YIELD) throw new Error("FLOW_RECONCILIATION_REQUIRES_YIELD");
