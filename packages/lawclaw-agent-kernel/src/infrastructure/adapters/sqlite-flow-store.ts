...
import type { DurableFlowStore, FlowStorageRules } from "../../contracts/flow-storage.ts";
...
import { FlowSqliteSession } from "./flow-sqlite-session.ts";
import { SqliteFlowCommands } from "./sqlite-flow-commands.ts";
import { SqliteFlowCommits } from "./sqlite-flow-commits.ts";

/** 真实SQLite Run存储；作用域在装配时固定，Core不持有数据库连接。 */
export class SqliteFlowStore implements DurableFlowStore {
	readonly #session: FlowSqliteSession;
	readonly #commands: SqliteFlowCommands;
	readonly #commits: SqliteFlowCommits;
	/** 建立共享连接并装配Run、命令、原子提交三项职责。 */
	constructor(path: string, scope: string, rules: FlowStorageRules, time: TimePort) {
		this.#session = new FlowSqliteSession(path, scope, rules, time);
		this.#commands = new SqliteFlowCommands(this.#session);
		this.#commits = new SqliteFlowCommits(this.#session);
	}
	...
	admit(input: AdvanceInput, parent?: AdvanceInput): void {
		if (
			...
			throw new Error("FLOW_ADMISSION_INVALID");
		...
		this.#session.db.transaction(() => {
			...
			if (existing) {
				if (existing.admission_digest !== digest) throw new Error("FLOW_ADMISSION_CONFLICT");
				return;
			}
	...
	#validateChildAdmission(child: AdvanceInput, parent: AdvanceInput): void {
		...
		if (
			...
			throw new Error("FLOW_CHILD_ADMISSION_DENIED");
